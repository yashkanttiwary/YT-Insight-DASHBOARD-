...
export const getKeys = (req: express.Request) => {
  return {
  ...
  app.get("/api/youtube-competitors", async (req, res) => {
    try {
      const keys = getKeys(req);
      const competitorsStr = req.headers["x-youtube-competitors"] as string;
  ...
  app.get("/api/youtube", async (req, res) => {
    try {
      ...
      let videoLimit = 50;
      try {
        const displayConfigStr = req.headers["x-display-config"] as string as string;
        if (displayConfigStr) {
           const display = JSON.parse(displayConfigStr);
           if (display.videoLimit) videoLimit = display.videoLimit;
        }
